...
Deno.serve(async () => {
  ...
  for (const p of staticPages) {
    urls.push(`  <url><loc>${SITE}${p.loc}</loc><changefreq>${p.changefreq}</changefreq><priority>${p.priority}</priority></url>`);
  }
  ...
  if (products) {
    for (const p of products) {
      ...
      urls.push(`  <url><loc>${SITE}/product/${p.slug}</loc><lastmod>${lastmod}</lastmod><changefreq>weekly</changefreq><priority>0.7</priority></url>`);
    }
  }
  ...
  if (categories) {
    for (const c of categories) {
      urls.push(`  <url><loc>${SITE}/category/${c.slug}</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>`);
    }
  }
  ...
  if (brands) {
    for (const b of brands) {
      urls.push(`  <url><loc>${SITE}/brand/${b.slug}</loc><changefreq>weekly</changefreq><priority>0.6</priority></url>`);
    }
  }
